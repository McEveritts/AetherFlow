...
function _repos() {
	...
	if [[ ${DISTRO} == Ubuntu ]]; then
		...
		LC_ALL=en_US.UTF-8 apt-add-repository ppa:ondrej/php -y >>"${OUTTO}" 2>&1
	fi

	if [[ ${DISTRO} == Debian ]]; then
        wget -qO - https://packages.sury.org/php/apt.gpg | gpg --dearmor -o /usr/share/keyrings/deb.sury.org-php.gpg
